...
async function main() {
  switch (command) {
    ...
    case 'watch': {
      ...
      if (maxIterations > 0) console.log(`   Iterations: ${maxIterations}`);
    ...
    case 'monitor': {
      ...
      if (subcommand === 'add') {
        ...
        for (const repo of added) console.log(`   + ${repo.label}`);
        break;
      }
      ...
      if (subcommand === 'list') {
        ...
        if (repos.length === 0) console.log('   (empty — use `threat-lab monitor add <path>` first)');
        break;
      }
