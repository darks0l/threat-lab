...
export async function scanTarget(options) {
    ...
    for (const file of files) {
        ...
        const threatIntel = intelRes.status === 'fulfilled' && intelRes.value != null ? intelRes.value : [];
        ...
        if (depRes.status === 'rejected')
            errors.push(`deps: ${depRes.reason}`);
        if (intelRes.status === 'rejected')
            errors.push(`intel: ${intelRes.reason}`);
