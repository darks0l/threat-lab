...
function buildModelabPrompt(scenarioId, scenarioName, txTraces, contractCode) {
...
function parseAnalysis(text) {
    ...
    const severity = (severityMatch?.[1]?.toLowerCase() ?? 'high');
    return {
...
export async function analyzeWithModelab(options) {
    ...
    for (const model of models) {
        try {
            const analysis = await directLLMCall(scenarioId, scenarioName, txTraces, contractCode, model);
            const report = {
                reportId: randomUUID(),
                scenarioId,
...
export function getBestAnalysis(results) {
    return [...results].sort((a, b) => b.report.confidence - a.report.confidence)[0];
}
//# sourceMappingURL=modelabIntegration.js.map