...
function buildResearchPrompt(finding) {
...
async function researchWithModel(finding, model) {
    if (!BANKR_API_KEY) {
...
export async function runDeepResearchBatch(options) {
...
function extractSolidityPatch(patchText, finding, contractCode) {
...
function insertReentrancyGuard(code) {
    if (/nonReentrant|reentrancyGuard/i.test(code))
        return code;
    let patch = code;
...
function addAccessControl(code) {
    if (/Ownable|AccessControl|onlyOwner/i.test(code))
        return code;
    let patch = code;
...
function describeDiff(patchCode, finding) {
    if (finding.title.includes('reentrancy')) {
...
export function formatDeepResearchReport(results) {
    const lines = [];
    lines.push('');
    lines.push('================================================================');
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        if (r.finding.packageName)
            lines.push(`    Package: ${r.finding.packageName}`);
        if (r.finding.contractFile)
            lines.push(`    File: ${r.finding.contractFile}`);
        lines.push('');
