...
async function getContractCode(contractName) {
 ...
 */
export async function runThreatLab(scenarioId, options = {}) {
    const { network = 'anvil', models = ['claude-sonnet-4-6'], submitToLibrary = true, submittedBy, } = options;
    console.log(`\n${'='.repeat(60)}`);
    ...
    const traces = executionResult.allTxHashes.map(hash => `tx: ${hash} | block: ${executionResult.startBlock}-${executionResult.endBlock}`);
    // 5. Run AI analysis
...
function printReport(report, scenarioName) {
    console.log(`\n${'─'.repeat(60)}`);
