 ...
 */
export async function runThreatIntel(options) {
    const { packages, braveApiKey = process.env.BRAVE_SEARCH_API_KEY ?? '', ghToken = process.env.GITHUB_TOKEN ?? '', daysLookback = DAYS_LOOKBACK, } = options;
    const results = [];
    ...
    for (const pkg of packages) {
        ...
        if (ghToken) {
            ...
            if (ghResult.findings.some(f => f.isAlert))
        ...
        if (braveApiKey) {
            ...
            if (braveResult.findings.some(f => f.isAlert))
                hasActiveExploit = true;
        }
        // ── Overall severity ──
        ...
        else if (recentFindings.length >= 3)
        ...
        console.log(`   ${sevIcon} ${pkg.name}@${pkg.version}: ` +
            `GH advisories: ${ghCount} | Web: ${webCount} → ${overallSeverity}`);
    }
    return results;
}
// ── GitHub Security Advisories ────────────────────────────────────────────────
async function searchGitHubAdvisories(packageName, token, days) {
    const cutoff = new Date();
    ...
    try {
        ...
        for (const adv of advisories) {
            ...
            const isNpm = adv.vulnerabilities?.some(v => v.package?.ecosystem?.toLowerCase() === 'npm'
            ...
            const isAlert = ALERT_KEYWORDS.some(k => adv.summary.toLowerCase().includes(k) || description.toLowerCase().includes(k));
            findings.push({
                title: adv.summary,
                url: adv.html_url,
...
// ── Brave Search ──────────────────────────────────────────────────────────────
async function searchBrave(packageName, version, apiKey, days) {
    const findings = [];
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - days);
        ...
        try {
            ...
            for (const result of web) {
                ...
                if (url.includes('twitter.com') || url.includes('x.com'))
                    source = 'twitter';
                else if (url.includes('github.com/advisory') || url.includes('github.com/security'))
                    source = 'github-advisory';
                else if (/security|exploit|cve|vulnerability|advisory/i.test(url))
...
function buildSummary(packageName, searches, findings, days) {
    const ghCount = searches.find(s => s.source === 'github-advisory')?.resultCount ?? 0;
