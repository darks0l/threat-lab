...
export async function auditDependencies(options) {
    const { projectPath, includeDev = true, socketDev: enableSocket = !!SOCKET_DEV_KEY, socketDevKey = SOCKET_DEV_KEY, runNpmAudit = true, } = options;
...
// ── npm audit ────────────────────────────────────────────────────────────────
async function performNpmAudit(projectDir, includeDev) {
    const devFlag = includeDev ? '--include=dev' : '';
    try {
        const { stdout } = await execAsync(`npm audit --json ${devFlag}`, { cwd: projectDir, timeout: 90_000, killSignal: 'SIGTERM' });
...
function parseNpmAuditResult(raw) {
    const vulns = raw.vulnerabilities ?? {};
    const meta = raw.metadata?.vulnerabilities;
    const vulnerabilities = Object.entries(vulns).map(([name, adv]) => ({
        id: adv.id ?? 0,
        moduleName: name,
...
// ── Socket.dev ────────────────────────────────────────────────────────────────
async function runSocketDevAnalysis(pkgJson, apiKey) {
    const allDeps = {
        ...
        try {
            ...
            for (const pkg of results) {
                ...
                if (info?.hasInstallScripts)
                    riskTypes.push('install-script');
                if (info?.hasLargeExpandos)
                    riskTypes.push('large-expandos');
                if (info?.hasTypoSquats)
                    riskTypes.push('typo-squat');
                if (info?.hasDependencyConfusion)
                    riskTypes.push('dependency-confusion');
                if (info?.hasNativeSnippets)
                    riskTypes.push('native-code');
                if (info?.hasSandboxEscapes)
                    riskTypes.push('sandbox-escape');
                if (info?.hasTelemetryScript)
                    riskTypes.push('telemetry');
                if (pkg.isArchived)
                    riskTypes.push('archived-package');
...
async function runOsintAnalysis(pkgJson) {
    const allDeps = {
        ...(pkgJson.dependencies ?? {}),
        ...(pkgJson.devDependencies ?? {}),
        ...(pkgJson.optionalDependencies ?? {}),
...
async function queryOsv(pkgName, version, findings, activeExploits) {
    try {
        const resp = await axios.post('https://osv.dev/v1/query', {
        ...
        for (const v of vulns) {
            const vv = v;
            const severity = mapOsvSeverity(vv.severity);
            const summary = String(vv.summary ?? '');
            const references = (vv.references ?? [])
                .map(r => r.url)
                .filter(Boolean);
            const published = String(vv.published ?? '');
            const isActive = ACTIVE_EXPLOIT_KEYWORDS.some(k => summary.toLowerCase().includes(k) ||
            ...
            findings.push({
                ...
                severity,
                title: summary || id,
                description: vv.details?.slice(0, 500),
...
async function queryNpmAdvisories(pkgName, version, findings, activeExploits) {
    try {
        const resp = await axios.get(`https://api.npmjs.org/advisories/${encodeURIComponent(pkgName)}`, { timeout: 10_000 });
        const advisories = resp.data?.advisories ?? [];
        for (const adv of advisories) {
            // Check if our version is affected
            if (!versionMatches(adv.vulnerable_versions, version))
                continue;
            const severity = mapNpmSeverity(adv.severity);
            const isActive = ACTIVE_EXPLOIT_KEYWORDS.some(k => adv.title.toLowerCase().includes(k) || adv.overview?.toLowerCase().includes(k));
            findings.push({
                packageName: pkgName,
                packageVersion: version,
                severity,
...
function versionMatches(vulnerableVersions, installed) {
    if (!vulnerableVersions || vulnerableVersions === '*')
        return true;
    try {
...
function mapOsvSeverity(s) {
    const l = (s ?? '').toLowerCase();
    if (l.includes('critical'))
        return 'critical';
    if (l.includes('high'))
        return 'high';
    if (l.includes('medium') || l === 'moderate')
        return 'medium';
    if (l.includes('low'))
        return 'low';
    return 'info';
...
// ── Pattern detection ─────────────────────────────────────────────────────────
function detectSuspiciousPatterns(pkgJson, npmAudit, socketDev, osintResult) {
    const patterns = [];
    const allDeps = {
        ...(pkgJson.dependencies ?? {}),
        ...(pkgJson.devDependencies ?? {}),
...
// ── Summary scoring ───────────────────────────────────────────────────────────
function buildAuditSummary(npmAudit, socketDev, osintResult, suspiciousPatterns, _pkgJson) {
    ...
    const totalScore = Math.min(vulnScore
        + maliciousCount * 50
        + highRiskCount * 15
...
function printAuditSummary(result) {
    ...
    if (result.npmAudit) {
        ...
        console.log(`\n   npm audit:`);
        if (b.critical > 0)
            console.log(`     🔴 Critical: ${b.critical}`);
        if (b.high > 0)
            console.log(`     🟠 High: ${b.high}`);
        if (b.medium > 0)
            console.log(`     🟡 Medium: ${b.medium}`);
        if (b.low > 0)
            console.log(`     🟢 Low: ${b.low}`);
