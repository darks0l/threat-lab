...
export const SCENARIOS = [
    {
        ...
        description: 'A vulnerable vault that allows recursive ETH withdrawals due to an external call made before state update. Classic entry-point for reentrancy attacks.',
        pattern: 'reentrancy',
        severity: 'critical',
        ...
        expectedOutcome: 'Attacker drains vault balance via reentrancy. Vault.withdraw() sends ETH before decrementing balances, enabling recursive calls. Full vault drained in a single transaction.',
        difficulty: 'beginner',
    ...
    },
    {
        ...
        description: 'A price oracle reads spot reserves from a Uniswap V2 pair without TWAP smoothing. A flash loan can inflate the price 10-100x in a single block.',
        ...
        expectedOutcome: 'Oracle reports manipulated price. Downstream protocols accept unfavorable rates. Profit extracted via arbitrage on the price differential.',
    ...
    },
    {
        ...
        description: 'Borrow 1M DAI via Balancer flash loan, execute arbitrage across two DEXs, return the loan — all in one transaction.',
        ...
        expectedOutcome: 'Profit extracted after flash loan fees. MEV bots typically sandwich this to capture the arbitrage spread.',
        difficulty: 'beginner',
        createdAt: new Date().toISOString(),
    ...
    },
    {
        ...
        description: 'A victim calls swap(sell ETH for USDC). An MEV bot detects the pending tx in the mempool, front-runs with a swap that drives the ETH price down, then back-runs with a swap that buys ETH back at the lower price — extracting value from the victim.',
    ...
    },
    {
        ...
        description: 'A malicious proposal is submitted to a Governor + Timelock. After voting passes and the timelock delay expires, the attacker executes the proposal to grant themselves the TIMELOCK_ADMIN_ROLE, seizing control of the protocol treasury.',
        ...
        expectedOutcome: 'Attacker uses governance to grant themselves the TIMELOCK_ADMIN_ROLE, enabling treasury drain or malicious protocol upgrades.',
        difficulty: 'advanced',
        createdAt: new Date().toISOString(),
    ...
    },
    {
        ...
        description: 'A user has a borrowing position near the liquidation threshold. An attacker manipulates the oracle price via a flash loan, making the collateral appear undercollateralized, then calls liquidate() to claim a bonus on the seized collateral.',
        pattern: 'oracle-manipulation',
        severity: 'high',
        exploitSteps: [
            {
