...
export function inferPattern(text) {
    ...
    if (/reentranc/.test(lower))
        return 'reentrancy';
    if (/oracle.*manip|price.*manip|spot.*price/.test(lower))
        return 'oracle-manipulation';
    if (/flash.?loan/.test(lower))
        return 'flash-loan-attack';
    if (/access.?control|permission|privilege/.test(lower))
        return 'access-control';
    if (/front.?run|sandwich/.test(lower))
        return 'sandwich-attack';
    if (/integer.*over|overflow|unchecked.*arith/.test(lower))
        return 'integer-overflow';
    if (/delegatecall|implementation.*storage/.test(lower))
        return 'delegatecall-injection';
    if (/permit.*replay|eip712.*permit/.test(lower))
        return 'permit-front-run';
    if (/liquidat|health.?factor|collateral.*seiz/.test(lower))
        return 'liquidation-attack';
    return 'unknown';
}
//# sourceMappingURL=patternSignatures.js.map