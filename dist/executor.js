...
// ── Contract deployment ───────────────────────────────────────────────────────
async function deployContract(name, args, wallet, provider) {
...
// ── Step execution ───────────────────────────────────────────────────────────
async function executeStep(step, contracts, wallet, provider) {
    const result = {
        step: step.step,
        action: step.action,
    ...
    try {
        switch (step.action) {
            ...
            case 'call': {
                ...
                if (!contract)
                    throw new Error(`Contract not found: ${step.target}`);
                ...
                let returnData;
                if (isViewRead) {
                    returnData = await c[step.method ?? ''].staticCall(...(step.args ?? []), { blockTag: 'latest' });
                }
                else {
                    const nonce = await provider.getTransactionCount(wallet.address, 'pending');
                    const tx = await c[step.method ?? ''](...(step.args ?? []), { nonce });
            ...
            case 'send': {
                ...
                const tx = await c[step.method ?? ''](...(step.args ?? []), { value, nonce });
                const receipt = await tx.wait();
                await mineBlock(provider);
                result.txHash = tx.hash;
                result.blockNumber = receipt?.blockNumber;
...
export async function executeScenario(scenario, options = {}) {
...
async function deployAndWait(factory, args, nonce) {
...
export async function getTxTrace(txHash, rpcUrl) {
